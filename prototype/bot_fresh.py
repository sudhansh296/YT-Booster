...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = ("📋 Read the terms and conditions of this bot\n\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not has_points(user_id):
        bot_link = f"https://t.me/{BOT_USERNAME}?start=ref_{user_id}"
        keyboard = [[InlineKeyboardButton("📤 Share & Get Free Points", url=f"https://t.me/share/url?url={bot_link}&text=Is+bot+se+kisi+ka+bhi+information+nikal+sakte+ho%21+English+mein+bhi+kaam+karta+hai%21")],
                    [InlineKeyboardButton("💳 Buy Premium", callback_data="buy_points")]]
