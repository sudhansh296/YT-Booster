...
class MainViewModel : ViewModel() {
    ...
    val isLoadingTransactions: StateFlow<Boolean> = _isLoadingTransactions

    private val _isLoadingLeaderboard = MutableStateFlow(true)
    val isLoadingLeaderboard: StateFlow<Boolean> = _isLoadingLeaderboard
    ...
    fun loadInit() {
        ...
        viewModelScope.launch {
            try {
                ...
                data.referral?.let { _referral.value = it }
            } catch (e: Exception) {
    ...
    fun submitWatchReward(coins: Int, onResult: (Boolean, String) -> Unit) {
        viewModelScope.launch {
            try {
                ...
                onResult(res.success, res.message ?: "Coins mil gaye!")
            } catch (e: Exception) {
                onResult(false, parseError(e))
            }
        }
    ...
    // ── Promo Videos ──────────────────────────────────────────
    private val _promoVideos = MutableStateFlow<List<com.ytsubexchange.data.PromoVideo>>(emptyList())
    val promoVideos: StateFlow<List<com.ytsubexchange.data.PromoVideo>> = _promoVideos

    fun loadPromoVideos() {
        viewModelScope.launch {
            try {
                val resp = RetrofitClient.api.getPromoVideos("Bearer $token")
                _promoVideos.value = resp.videos
            } catch (e: Exception) { }
        }
    }

    fun claimPromoReward(videoId: String, onResult: (Boolean, String, Int) -> Unit) {
        viewModelScope.launch {
            try {
                val res = RetrofitClient.api.claimPromoReward("Bearer $token", mapOf("videoId" to videoId))
                if (res.success) loadProfile()
                onResult(res.success, if (res.success) "+${res.coins} coins!" else (res.message ?: "Already claimed"), res.coins)
            } catch (e: Exception) {
                onResult(false, parseError(e), 0)
            }
        }
    }

    fun submitVideoOrder(videoUrl: String, coins: Int, videoType: String, onResult: (Boolean, String) -> Unit) {
        viewModelScope.launch {
            try {
